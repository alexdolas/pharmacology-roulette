  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-900 via-indigo-900 to-black flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-40 pointer-events-none">
        ...
        <div className="absolute top-40 -right-40 w-80 h-80 bg-indigo-500 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-96 h-96 bg-amber-400 rounded-full blur-3xl opacity-40" />
      </div>
      ...
      <div className="relative max-w-md w-full bg-gradient-to-b from-indigo-950/80 via-slate-950/90 to-black/90 rounded-3xl border border-indigo-500/40 shadow-[0_0_80px_rgba(129,140,248,0.6)] p-5 md:p-6 backdrop-blur-xl z-10">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl md:text-2xl font-extrabold text-white drop-shadow-[0_0_20px_rgba(129,140,248,0.8)] tracking-tight flex items-center gap-2">
              <Award className="w-6 h-6 text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.9)]" />
              Toxicology Roulette
            </h1>
          ...
          <div className="relative w-[320px] h-[320px] md:w-[380px] md:h-[380px]">
            ...
            >
              ...
              {drugPairs.map((pair, index) => {
                ...
                let rotation = (midRad * 180) / Math.PI;
      ...
      {showModal && currentDrug && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-30 animate-fadeIn">
          <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 border border-indigo-500/60 rounded-3xl shadow-[0_0_50px_rgba(59,130,246,0.8)] p-5 md:p-6 max-w-md w-full animate-scaleIn relative overflow-hidden">
            <div className="absolute -top-20 -right-16 w-40 h-40 bg-indigo-600/40 rounded-full blur-3xl" />
            <div className="absolute bottom-[-60px] left-[-20px] w-40 h-40 bg-amber-500/30 rounded-full blur-3xl" />

            <div className="relative">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl md:text-2xl font-extrabold text-white flex items-center gap-2">
                  <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-indigo-600/80 text-lg shadow-lg shadow-indigo-900/80">
                    💊
                  </span>
                  Match the antidote
                </h2>
                <button
                  ...
                  className="text-slate-300 hover:text-white text-sm px-2 py-1 rounded-lg bg-slate-900/80 border border-slate-700/80"
                >
                  Close
                </button>
              </div>

              <div className="bg-slate-900/80 border border-slate-700/80 rounded-2xl p-3.5 mb-4 shadow-inner shadow-black/60">
                <div className="text-xs text-slate-300 uppercase tracking-wide mb-1.5">
                  Toxin
                </div>
                <div className="flex items-center justify-between gap-2">
                  <div className="text-lg font-bold text-amber-200">{currentDrug.drug}</div>
                  <div className="text-[10px] px-2 py-1 rounded-full bg-indigo-700/70 text-indigo-100 border border-indigo-400/70">
                    Pick 1 antidote
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                {shuffledOptions.map((option) => {
                  ...
                  let bgClass =
                    'bg-slate-900/80 border-slate-700/80 hover:bg-slate-800/80 hover:border-indigo-400/70';
                  let textClass = 'text-slate-100';
                  let borderClass = 'border';

                  if (showResult && isCorrect) {
                    bgClass = 'bg-emerald-900/80 border-emerald-400/80';
                    textClass = 'text-emerald-100';
                  } else if (showResult && isSelected && !isCorrect) {
                    bgClass = 'bg-rose-900/80 border-rose-500/80';
                  ...
                  return (
                    <button
                      ...
                      disabled={showResult}
                      className={`w-full text-left px-3.5 py-2.5 rounded-2xl ${bgClass} ${borderClass} ${textClass} text-sm md:text-base transition-all flex items-center justify-between gap-2 shadow-md shadow-black/60`}
                    >
                      <span className="flex-1">{option}</span>
                      {showResult && isCorrect && (
                        <span className="text-emerald-300 text-sm font-semibold">Correct</span>
                      )}
                      {showResult && isSelected && !isCorrect && (
                        <span className="text-rose-300 text-sm font-semibold">Wrong</span>
                      )}
                    </button>
                  );
                })}
              ...
              {showResult && (
                <div className="mt-3.5 text-xs text-slate-300">
                  Correct antidote:&nbsp;
                  <span className="font-semibold text-emerald-300">
                    {currentDrug.antidote}
                  </span>
                </div>
              )}
            </div>
...
export default PharmacologyRoulette;
